  ...
  return (
    <div className="w-full">
      ...
      <input
        ...
        onBlur={handleBlur}
        placeholder="Search tests..."
        className="w-full p-3 sm:p-4 border border-teal-400 rounded-full
                   text-base sm:text-lg md:text-xl
                   focus:outline-none focus:ring-2 focus:ring-teal-400 focus:shadow-lg
                   focus:bg-white focus:shadow-teal-400
                   transition-all ease-linear duration-500"
      />
      ...
      {suggestions.length > 0 && (
        ...
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] text-left border-collapse bg-teal-50">
              <thead>
                <tr className="bg-blue-950 border-b text-teal-400 text-sm sm:text-base">
                  <th className="p-2 sm:p-3">ID</th>
                  <th className="p-2 sm:p-3">Test Name</th>
                  <th className="p-2 sm:p-3">Original Price</th>
                  <th className="p-2 sm:p-3">Membership Discounted</th>
                  <th className="p-2 sm:p-3">General Discounted</th>
                </tr>
              </thead>
              <tbody>
                {suggestions.map((test) => (
                  <tr
                    ...
                    onClick={() => handleSuggestionClick(test)}
                  >
                    <td className="p-2 sm:p-3 font-semibold text-blue-950">
                      {test.id}
                    </td>
                    <td className="p-2 sm:p-3 font-semibold text-blue-950">
                      {test.name}
                    ...
                    {test.membership_price && (
                      <td className="p-2 sm:p-3 font-bold text-teal-400">
                        {test.membership_price === test.price
                          ? null
                          : "Rs." + test.membership_price}
                      </td>
      ...
      {/* Selected Tests */}
      {values.selectedTest.length > 0 && (
        <div className="mt-4 text-teal-500 bg-gray-50 border border-teal-400 p-3 sm:p-4 rounded-xl">
          <h1 className="mb-5 text-base sm:text-lg font-semibold">
            Selected Tests
          </h1>
            ...
            {values.selectedTest.map((test, index) => {
              ...
              return (
                <div
                  key={test.id}
                  className="bg-white p-3 rounded-xl border border-teal-400
                             flex flex-col gap-2 sm:gap-3 items-start shadow-xl"
                >
                  <h1 className="text-sm sm:text-base text-blue-950 font-semibold">
                    {test.name}
                  </h1>
                  <h1 className="text-sm sm:text-base text-blue-950 m-0">
                    Price: Rs. {test.discounted_price ?? test.price}
                  </h1>
                  <Button
                    bname="Delete"
          ...
          <div className="mt-6 sm:mt-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <h1 className="text-base sm:text-lg md:text-xl flex flex-col sm:flex-row justify-between w-full gap-3">
              <div>
                <span className="text-blue-950 font-bold">Total Price:</span>{" "}
                <span className="font-bold drop-shadow-2xl">
                  Rs.{props.offerPrice ?? total_price}
