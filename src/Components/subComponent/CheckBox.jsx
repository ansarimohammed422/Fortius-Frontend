  ...
  return (
    <>
      {props.category.map((service, index) => (
        <div
          key={service.id}
          className="relative w-full max-w-[250px] sm:max-w-[300px] md:max-w-[350px] lg:max-w-[400px]"
        >
          <input
            className="peer hidden"
            id={`checkbox_${index + 1}`}
          ...
          ></span>
          <label
            className={`flex h-full cursor-pointer flex-col rounded-lg p-3 sm:p-4 bg-white peer-checked:bg-teal-400 peer-checked:border peer-checked:border-blue-950 peer-checked:shadow-lg peer-checked:shadow-gray-400 peer-checked:text-blue-950 border border-teal-400 transition-all ease-linear duration-500`}
            htmlFor={`checkbox_${index + 1}`}
          >
            <span className="mt-1 sm:mt-2 font-bold text-blue-950 text-sm sm:text-base md:text-lg">
              {service.name}
            </span>
            <span className="text-[10px] sm:text-xs uppercase">Tests</span>
          </label>
        </div>
      ))}
    </>
