...
//               props.userMembership.plan === plan.name ? (
//                 <button
...
const MembershipPlans = (props) => {
  ...
  return (
    <div className="p-6 sm:p-8 md:p-10 bg-white h-fit flex flex-col items-center">
      <h2 className="text-2xl sm:text-3xl md:text-3xl font-bold text-blue-950 mb-6 text-center">
        Choose Your Membership Plan
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full max-w-5xl">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className="bg-teal-50 shadow-lg rounded-lg p-6 border border-teal-400 hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
          >
            <div>
              <h3 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-2">
                {plan.name}
              </h3>
              <p className="text-gray-600 text-md sm:text-lg mb-1">
                Price:{" "}
                <span className="font-bold text-teal-600">₹{plan.price}</span>
              </p>
