  ...
  if (loading) {
    return (
      <div className="container mx-auto p-4 text-center">
        <h1 className="text-2xl font-semibold text-blue-950">Loading Offers...</h1>
      </div>
    );
  }
    ...
    <div className="bg-white text-gray-800">
      ...
      <section className="relative bg-[url('./assets/New_teal.jpg')] bg-cover bg-center h-screen text-white p-40 flex items-center justify-center">
        <div className="text-center bg-teal-400/30 backdrop-blur-lg rounded-xl p-28 border-2 border-blue-950 shadow-lg">
          <h1 className="text-9xl font-extrabold text-blue-950">Active Offers</h1>
          <p className="mt-4  text-2xl text-blue-950">
          Limited Time Offers! Unwrap Incredible Savings Today!          </p>
        </div>
      </section>
  ...
  <div className="container mx-auto px-6">
    ...
    <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
      {offers.map((offer) => (
        ...
        >
          <h2 className="text-2xl font-bold text-blue-950 mb-4 tracking-wide">{offer.description}</h2>
          <p className="text-lg text-gray-600 mb-4">
            <strong className="text-teal-400">Discount:</strong> {offer.discount_percentage}% off
          </p>
          <p className="text-lg text-gray-600 mb-4">
            <strong className="text-teal-400">Valid from:</strong> {new Date(offer.start_date).toLocaleDateString()} <strong className='test-teal-400'>Till{' '}</strong>
