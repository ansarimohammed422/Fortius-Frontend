...
const Services = () => {
  const services = [
    ...
    },
    {
      ...
      image: "./assets/serology.jpg",
      description:
        "Serology examines the blood serum to detect antibodies and antigens, providing insight into infections, autoimmune diseases, and allergies. It plays a significant role in diagnosing diseases such as HIV, hepatitis, and malaria. Serology tests are used to assess immune responses and are also essential in blood transfusion and organ transplantation for compatibility testing. By detecting antibodies, it helps identify past and present infections.",
      tests: [
        "HIV Antibody Test",
        "Hepatitis B Surface Antigen",
        "Malaria Parasite Antigen Test",
    ...
    },
    {
      ...
      image: "./assets/Radiology.jpg",
      description:
        "Radiology uses imaging techniques like X-rays, CT scans, MRI, and ultrasound to diagnose diseases. It helps visualize internal structures of the body to detect abnormalities such as fractures, tumors, and infections. Radiology plays a crucial role in identifying conditions like cancers, cardiovascular diseases, and bone disorders. In addition to diagnostic imaging, interventional radiology offers minimally invasive procedures for biopsies, catheter placements, and treatments like stent placements.",
      tests: ["X-ray Imaging", "CT Scan", "MRI Scan"],
    },
  ];
  ...
  return (
    <div className="bg-white text-gray-800">
      {/* Hero Section */}
      <section className="relative bg-[url('./assets/New_teal.jpg')] bg-cover bg-center h-[90vh] flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="text-center bg-teal-400/30 backdrop-blur-lg rounded-xl p-12 sm:p-16 md:p-20 border-2 border-blue-950 shadow-lg">
          <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold text-blue-950">
            Fortius Services
          </h1>
          <p className="mt-4 mb-8 sm:mb-10 text-lg sm:text-xl md:text-2xl text-blue-950">
            Explore our comprehensive range of diagnostic services to guide you
            toward better health.
          </p>
          <div className="mt-6">
            <MyLinks
              LName="Book Now"
              path="/Appointment"
              customClass="text-lg sm:text-xl bg-blue-950 border-2 shadow-none hover:shadow-xl hover:border-none hover:!text-blue-950 border-teal-400 px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-transparent hover:text-teal-400"
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-12 sm:py-16 md:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-blue-950 text-center mb-8 sm:mb-10 md:mb-12">
            Our Specialized Services
          </h2>
          <div className="space-y-12 sm:space-y-16">
            {services.map((service, index) => (
              <div
                key={index}
                className="bg-white p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl shadow-md sm:shadow-lg border border-teal-300 hover:shadow-xl transition-all duration-300"
              >
                <div className="flex flex-col md:flex-row items-center md:items-start md:space-x-8 space-y-4 md:space-y-0">
                  {/* Image */}
                  <div
                    className="w-full md:w-1/3 h-48 sm:h-56 md:h-64 rounded-lg bg-cover bg-center shadow-md sm:shadow-lg"
                    style={{ backgroundImage: `url(${service.image})` }}
                  ></div>

                  {/* Service Info */}
                  <div className="w-full md:w-2/3">
                    <h3 className="text-2xl sm:text-3xl font-semibold text-blue-950 mb-3 sm:mb-4">
                      {service.name}
                    </h3>
                    <p className="text-base sm:text-lg md:text-xl text-blue-950/70 mb-4 sm:mb-6">
                      {service.description}
                    </p>
                    ...
                    <button
                      onClick={() => toggleShowMore(index)}
                      className={`font-bold px-4 sm:px-5 py-1.5 sm:py-2 rounded-lg border-2 text-base sm:text-lg transition-all duration-500 ease-linear ${
                        showMoreStates[index]
                          ? "bg-blue-950 text-teal-400 hover:bg-teal-400 hover:text-blue-950 hover:border-teal-400"
                          : "bg-teal-400 text-blue-950 border-blue-950 hover:bg-blue-950 hover:text-teal-400"
                      }`}
                    ...
                    {/* Expanded Content */}
                    <div
                      className={`mt-4 sm:mt-6 overflow-hidden transition-all duration-500 ${
                        showMoreStates[index]
                          ? "max-h-screen opacity-100"
                          : "max-h-0 opacity-0"
                      }`}
                    >
                      <div className="p-4 sm:p-6 bg-teal-50 rounded-lg shadow-sm sm:shadow-md border border-blue-950">
                ...
                <div className="mt-6 sm:mt-8 md:mt-10">
                  <h4 className="text-2xl sm:text-3xl md:text-3xl font-bold text-blue-950 mb-4 sm:mb-6 text-center md:text-left">
                    Tests We Offer
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                    {service.tests.map((test, idx) => (
                      <div
                        key={idx}
                        className="bg-teal-50 p-4 sm:p-6 rounded-lg shadow-md sm:shadow-lg border border-teal-300 hover:bg-teal-100 hover:scale-105 transform transition-all duration-500 text-center"
                      >
                        <p className="text-lg sm:text-xl md:text-xl font-medium text-blue-950">
                          {test}
                        </p>
                      </div>
                    ))}
