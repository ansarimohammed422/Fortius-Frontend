...
const About = () => {
  return (
    <div className="bg-white text-gray-800">
      {/* Hero Section */}
      <section className="relative bg-[url('./assets/New_teal.jpg')] bg-cover bg-center h-screen flex items-center justify-center">
        <div className="text-center bg-teal-400/30 backdrop-blur-lg rounded-3xl p-8 sm:p-12 md:p-16 lg:p-20 border-2 border-blue-950 shadow-2xl">
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-9xl font-black text-blue-950">
            About Us
          </h1>
          <p className="mt-4 text-lg sm:text-xl md:text-2xl text-blue-950 max-w-4xl mx-auto">
            ...
            patient-first approach, we ensure precision and care in every test,
      ...
      <section className="bg-white py-12 sm:py-16">
        <h2 className="text-4xl sm:text-5xl text-center font-bold text-blue-950 mb-12">
          Why Us?
        </h2>
        <div className="container mx-auto px-4 sm:px-6 lg:px-12 flex flex-wrap justify-center gap-8">
          {[
            {
            ...
            <div
              key={index}
              className="w-40 sm:w-48 flex flex-col items-center transform transition duration-500 hover:scale-110"
            >
              <div className="relative flex justify-center items-center w-32 h-32">
                <img
                  src={item.img}
                  alt={item.title}
                  className="w-full h-full object-contain filter brightness-75 rounded-full"
                />
                <div className="absolute inset-0 bg-teal-400 mix-blend-screen rounded-full"></div>
              </div>
              <h3 className="mt-3 text-center text-lg font-semibold text-blue-950">
                {item.title}
              </h3>
            </div>
          ))}
      ...
      <section className="bg-white py-12 sm:py-16 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-12 flex flex-col gap-12">
          {/* About Image */}
          <img
            src={myImage}
            alt="About"
          ...
          {[
            {
              title: "Story",
              text: `"Our Stories" at Fortius encapsulates our journey of dedication and innovation in delivering reliable diagnostic solutions. Founded with a vision to empower healthcare through precision and trust, we have grown into a platform that combines cutting-edge technology with compassionate service. Each milestone reflects our commitment to providing accurate diagnostics, fostering healthier communities, and building lasting relationships with those we serve. At Fortius, every story is a testament to the lives we touch and the trust we earn.`,
            },
            {
              title: "Mission",
              text: `"Our Mission" at Fortius is to revolutionize healthcare diagnostics by providing accurate, accessible, and timely solutions that empower individuals to take charge of their health. We are committed to leveraging advanced technology and expert care to ensure precision, reliability, and affordability in every service we offer. Guided by our dedication to excellence, we aim to foster healthier communities and set new standards in diagnostic care.`,
            },
            {
              title: "Approach",
              text: `"Our Approach" at Fortius is rooted in innovation, precision, and patient-centric care. We blend state-of-the-art diagnostic technology with a seamless user experience to deliver reliable results efficiently. Our team emphasizes transparency and attention to detail, ensuring every process is handled with the utmost professionalism. By fostering a culture of continuous improvement and collaboration, we aim to make healthcare diagnostics more accessible, convenient, and trustworthy for everyone.`,
            },
          ].map((item, index) => (
            <div
              key={index}
              className="flex flex-col md:flex-row justify-between gap-6 border border-blue-950 bg-teal-50 p-6 sm:p-10 rounded-xl shadow-lg transition-transform duration-300 hover:scale-105 hover:shadow-2xl"
            >
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold text-blue-950 flex-1">
                {item.title}
              </h2>
              <p className="text-blue-950/70 flex-1 text-lg sm:text-xl">
                {item.text}
              </p>
            </div>
          ))}
